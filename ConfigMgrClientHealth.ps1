...
Begin {
    ...
    $WriteHostMetadata = New-Object System.Management.Automation.CommandMetadata (Get-Command Write-Host)	    # #Write-Verbose
    $WriteHostBinding = [System.Management.Automation.ProxyCommand]::GetCmdletBindingAttribute($WriteHostMetadata)	    # $WriteVerboseMetadata = New-Object System.Management.Automation.CommandMetadata (Get-Command Write-Verbose)
    $WriteHostParams = [System.Management.Automation.ProxyCommand]::GetParamBlock($WriteHostMetadata)	    # $WriteVerboseBinding = [System.Management.Automation.ProxyCommand]::GetCmdletBindingAttribute($WriteVerboseMetadata)
    $WriteHostWrapped = {Microsoft.Powershell.Utility\Write-Host @PSBoundParameters; Out-LogFile -Text $Object}	    # $WriteVerboseParams = [System.Management.Automation.ProxyCommand]::GetParamBlock($WriteVerboseMetadata)
    ${Function:Write-Host} = '{0}param({1}) {2}' -f $WriteHostBinding, $WriteHostParams, $WriteHostWrapped	    # $WriteVerboseWrapped = {Microsoft.Powershell.Utility\Write-Verbose @PSBoundParameters; Out-LogFile -Text $Message}
    ...
    Function Get-ProvisioningMode {
        ...
        if ($provisioningMode -eq 'true') { $obj = $true } else { $obj = $false }
        Write-Output $obj
    }
    ...
    Function Get-Computername {
        if ($PowerShellVersion -ge 6) { $obj = (Get-CimInstance Win32_ComputerSystem).Name } else { $obj = (Get-WmiObject Win32_ComputerSystem).Name }
        Write-Output $obj
    }

    Function Get-LastBootTime {
        if ($PowerShellVersion -ge 6) { $wmi = Get-CimInstance Win32_OperatingSystem } else { $wmi = Get-WmiObject Win32_OperatingSystem }
    ...
    Function Get-LastInstalledPatches {
        ...
        if ($PowerShellVersion -ge 6) { 
            $Hotfix =   Get-CimInstance -ClassName Win32_QuickFixEngineering | 
            ...
            $Hotfix =   Get-HotFix | 
                        Select-Object @{l = 'InstalledOn'; e = { [DateTime]::Parse($_.psbase.properties['installedon'].value, $([System.Globalization.CultureInfo]::GetCultureInfo('en-US'))) } } 
        }
    ...
    Function Test-DNSConfiguration {
        ...
        if (($OSName -notlike '*Windows 7*') -and ($OSName -notlike '*Server 2008*')) {
            ...
            try {
                ...
                $dnsAddressList = Resolve-DnsName -Name $fqdn -Server ($dnsServers | Select-Object -First 1) -Type A -DnsOnly | Where-Object {$_.Name -eq $fqdn} | Select-Object -ExpandProperty IPAddress            } catch {
        ...
        switch ($obj) {
            $false {
                ...
                if ($fix -eq 'true') {
                    ...
                    if ($PowerShellVersion -ge 4) { Register-DnsClient | Out-Null } else { ipconfig /registerdns | Out-Null }
                    Write-Host $text
                    $log.DNS = $logFail
    ...
    Function Test-DiskSpace {
        ...
        if ($PowerShellVersion -ge 6) { 
            $driveC = Get-CimInstance -Class Win32_LogicalDisk | 
                        Where-Object { $_.DeviceID -eq "$env:SystemDrive" } | 
                        Select-Object FreeSpace, Size 
        } else { $driveC = Get-WmiObject -Class Win32_LogicalDisk | 
                            Where-Object { $_.DeviceID -eq "$env:SystemDrive" } | 
                            Select-Object FreeSpace, Size 
        }
        #$freeSpace = (($driveC.FreeSpace / $driveC.Size) * 100)
